...
unsigned char *msgDataPtr2 = (unsigned char *)&msgData2;                         // Make a pointer to msgData so we can access individual bytes
                                                    // 8 byte buffer for RxObj[1] message data
...
unsigned int msgData5;                                                           // The message data is four bytes long which we can allocate as an int32
unsigned char *msgDataPtr5 = (unsigned char *)&msgData5;                         // Make a pointer to msgData so we can access individual bytes
                                                  // 8 byte buffer for RxObj[1] message data

unsigned int msgData6;                                                           // The message data is four bytes long which we can allocate as an int32
unsigned char *msgDataPtr6 = (unsigned char *)&msgData6;                         // Make a pointer to msgData so we can access individual bytes
                                                     // 8 byte buffer for RxObj[1] message data

unsigned int msgData7;                                                           // The message data is four bytes long which we can allocate as an int32
unsigned char *msgDataPtr7 = (unsigned char *)&msgData7;                         // Make a pointer to msgData so we can access individual bytes
                                                    // 8 byte buffer for RxObj[1] message data
...
void CAN_Transmit(uint8_t data[4], uint8_t msgSelect){
	if(msgSelect==1){
		...
		CANMessageSet(CAN0_BASE, 8, &TxObj[0], MSG_OBJ_TYPE_TX); // send as msg object 1
		printf("Message sent.\n");
	}
	else if(msgSelect==2){
		...
		CANMessageSet(CAN0_BASE, 9, &TxObj[1], MSG_OBJ_TYPE_TX); // send as msg object 2
		printf("Message sent.\n");
	}
	else if(msgSelect==3){
		...
		CANMessageSet(CAN0_BASE, 10, &TxObj[2], MSG_OBJ_TYPE_TX); // send as msg object 2
		printf("Message sent.\n");
	}
		else if(msgSelect==4){
		...
		CANMessageSet(CAN0_BASE, 11, &TxObj[3], MSG_OBJ_TYPE_TX); // send as msg object 2
		printf("Message sent.\n");
	}
		else if(msgSelect==5){
		...
		printf("Sending message 2:\tByte 1: %d\tByte 2: %d\tByte 3: %d\t Byte 4: %d\n", msgDataPtr5[0], msgDataPtr5[1], msgDataPtr5[2], msgDataPtr5[3]); // write colour to UART for debugging	
		CANMessageSet(CAN0_BASE, 12, &TxObj[4], MSG_OBJ_TYPE_TX); // send as msg object 2
		printf("Message sent.\n");
	}
		else if(msgSelect==6){
		...
		printf("Sending message 2:\tByte 1: %d\tByte 2: %d\tByte 3: %d\t Byte 4: %d\n", msgDataPtr6[0], msgDataPtr6[1], msgDataPtr6[2], msgDataPtr6[3]); // write colour to UART for debugging	
		CANMessageSet(CAN0_BASE, 13, &TxObj[5], MSG_OBJ_TYPE_TX); // send as msg object 2
		printf("Message sent.\n");
	}
		else if(msgSelect==7){
		...
		printf("Sending message 2:\tByte 1: %d\tByte 2: %d\tByte 3: %d\t Byte 4: %d\n", msgDataPtr7[0], msgDataPtr7[1], msgDataPtr7[2], msgDataPtr7[3]); // write colour to UART for debugging	
		CANMessageSet(CAN0_BASE, 14, &TxObj[6], MSG_OBJ_TYPE_TX); // send as msg object 2
		printf("Message sent.\n");
	}		
